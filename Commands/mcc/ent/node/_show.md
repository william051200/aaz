...
- Show an MCC Enterprise cache node
    ```bash
        mcc ent node show --mcc-resource-name MyMccResource --cache-node-name MyCacheNode --resource-group MyResourceGroup
    ```

- Show a cache node with the full set of details
    ```bash
        mcc ent node show --mcc-resource-name MyMccResource --cache-node-name MyCacheNode --resource-group MyResourceGroup --expand-output
    ```
